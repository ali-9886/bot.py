...
class AliveHTTPServer(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write("🤖 البوت يعمل ومحصن ضد الموت الصامت!".encode('utf-8'))
...
# تحديد مسارات معالجة ذكية لمنع تعليق المسار الرئيسي
...
# =====================================================================
# 📥 استقبال الأوامر والترحيب اللطيف
# =====================================================================
...
def handle_download(message):
    ...
    if any(domain in url_lower for domain in supported_domains):
        ...
        # تحويل روابط إنستغرام وتويتر لتخفيف الضغط تماماً عن السيرفر
        if "instagram.com" in url_lower:
            try:
                proxy_url = url.replace("instagram.com", "ddinstagram.com").replace("www.", "")
                bot.send_video(message.chat.id, proxy_url, reply_to_message_id=message.message_id, caption="تفضل يا طيب! 🍿✨")
        ...
        # التحميل الآمن لمنع امتلاء الذاكرة
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            'no_cache_dir': True, # منع حفظ الكاش نهائياً لحماية الرام
            'socket_timeout': 8,   # قطع الاتصال فوراً لو علق البروكسي
        }
...
# =====================================================================
# 🚀 تشغيل البوت الآمن وتنظيف التعليق
# =====================================================================
